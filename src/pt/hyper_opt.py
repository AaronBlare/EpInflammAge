...
def get_model_config_trial(
    ...
    if model_config_default._model_name == 'GANDALFModel':
        ...
        model_config['k'] = trial.suggest_int('k', 2, 4)
        model_config['dropout_rate'] = trial.suggest_float('dropout_rate', 0.0, 0.25)
        model_config['learning_rate'] = trial.suggest_float('learning_rate', 0.00001, 1.0, log=True)
...
def get_data_config_trial(
    ...
    data_config['continuous_feature_transform'] = trial.suggest_categorical(
        ...
        [None, "yeo-johnson", "quantile_normal", "quantile_uniform"]
    )
...
def train_hyper_opt_cv(
        ...
        trainer_config_default: Union[TrainerConfig, str],
        experiment_config_default: Optional[Union[ExperimentConfig, str]],
        train_validation_sets: List[Tuple[pd.DataFrame, pd.DataFrame]],
        test: pd.DataFrame,
        min_lr: float = 1e-8,
        max_lr: float = 1,
        num_training: int = 100,
    ...
    Args:
        ...
        train_validation_sets (List[Tuple[pd.DataFrame, pd.DataFrame]]): 
            The list of train-validation folds.
        ...
        max_lr (Optional[float], optional): 
            maximum learning rate to investigate

        num_training (Optional[int], optional): 
        ...
        handle_oom (bool): 
            If True, will try to handle OOM errors elegantly.

        ignore_oom (bool, optional): 
            If True, will ignore the Out of Memory error and continue with the next model.

        verbose (bool, optional): 
            If True, will print the progress.

        suppress_lightning_logger (bool, optional): 
            If True, will suppress the lightning logger.
        
        seed (int, optional): 
            Random seed for datamodule
    ...
    res_dict = {}
    for fold_id, fold in enumerate(train_validation_sets):
        train = fold[0]
        validation = fold[1]
        ...
        datamodule = tabular_model.prepare_dataloader(train=train, validation=validation, seed=seed)
        ...
        if oom_handler.oom_triggered:
            if not ignore_oom:
                ...
                for part in opt_parts:
                    for metric_pair in opt_metrics:
                        res_dict[fold_id][f"{part}_{metric_pair[0]}"] = np.inf if metric_pair[1] == 'minimize' else -np.inf
                res_dict[fold_id]["model"] = tabular_model.name + " (OOM)"
        else:
            if (
                    tabular_model.trainer.early_stopping_callback is not None
                    and tabular_model.trainer.early_stopping_callback.stopped_epoch != 0
            ):
                res_dict[fold_id]["epochs"] = tabular_model.trainer.early_stopping_callback.stopped_epoch
            else:
                res_dict[fold_id]["epochs"] = tabular_model.trainer.max_epochs
            ...
            for m_name in metrics_names:
                train_metrics[m_name.replace('test', 'train')] = train_metrics.pop(m_name)
            res_dict[fold_id].update(train_metrics)
            ...
            for m_name in metrics_names:
                validation_metrics[m_name.replace('test', 'validation')] = validation_metrics.pop(m_name)
            res_dict[fold_id].update(validation_metrics)

            # Update results with test metrics
            res_dict[fold_id].update(tabular_model.evaluate(test=test, verbose=False)[0])
            
            # Save

            res_dict[fold_id]["time_taken"] = time.time() - start_time
            res_dict[fold_id]["time_taken_per_epoch"] = res_dict[fold_id]["time_taken"] / res_dict[fold_id]["epochs"]

            if verbose:
                logger.info(f"Finished Training {tabular_model.name}")
                logger.info("Results:" f" {', '.join([f'{k}: {v}' for k, v in res_dict[fold_id].items()])}")

            res_dict[fold_id]["model_params"] = model_config_trial
            res_dict[fold_id]["data_params"] = data_config_trial
            res_dict[fold_id]["optimizer_params"] = optimizer_config_trial

            if tabular_model.trainer.checkpoint_callback:
                try:
                    res_dict[fold_id]["checkpoint"] = tabular_model.trainer.checkpoint_callback.best_model_path
                    save_dir = str(Path(res_dict[fold_id]["checkpoint"]).parent).replace('\\', '/') + '/' + Path(res_dict[fold_id]["checkpoint"]).stem
                    tabular_model.save_model(save_dir)
                    os.remove(res_dict[fold_id]["checkpoint"])
                except:
                    pass

            trials_results.append(res_dict[fold_id])
    ...
    for part in opt_parts:
        for metric_pair in opt_metrics:
            metric_val = 0.0
            for fold_id, fold_res_dict in res_dict.items():
                metric_val += fold_res_dict[f"{part}_{metric_pair[0]}"]
            metric_val = metric_val / len(res_dict)
            result.append(metric_val)
